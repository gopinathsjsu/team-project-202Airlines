const SQL_BOOKING = {
  ...
  GET_BOOKING_FOR_UPDATE:
    "select class, traveller_cnt, booking_id,DATE_FORMAT(booking_date,'%Y-%m-%d') as booking_date,DATE_FORMAT(flight_date,'%Y-%m-%d') as dep_date,DATE_FORMAT(arr_date,'%Y-%m-%d') as arr_date,airport_code_src as src,airport_code_dst as dst,Booking.price,Booking.milesused as miles,status \
    from Booking,Flight \
  ...
  UPDATE_BOOKING:
    "Update  Booking set flight_id = ? ,  price = ? , milesused = ?, class = ? where booking_id = ? ",
  // INSERT_TRAVELLERS:
  //   "insert into Traveller (flight_id, first,middle,last,nationality,booking_id, gender, age, seatId) values ?",
  DELETE_TRAVELLERS: "delete from Traveller where booking_id = ? ",
};
